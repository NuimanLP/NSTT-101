...
function Detail(props) {
  ...
  const [editTimeCount, setEditTimeCount] = useState(null);
  ...
  const handleEditTimeCount = async () => {
    try {
      // Update the tour with edited TimeCount
      const response = await axios.put(`http://localhost:1337/api/tours/${props.id}`, {
        data: {
          TimeCount: editTimeCount,
          // ... other properties
        }
      });
      ...
      console.error("Error editing TimeCount:", error.response);
      console.log('Error details:', error.response.data);
    }
  };
        ...
        <div className='Detail' style={{ marginTop: '30px' }}>
          ...
          {data && (
            <div key={data.id} className='TextDetail0' style={{ marginLeft: '40px' }}>
              {isEditing && editTimeCount !== null ? (
                <div>
                  <input
                    type="text"
                    value={editTimeCount}
                    onChange={(e) => setEditTimeCount(e.target.value)}
                  />
                  <button className='saveButton' onClick={handleEditTimeCount}>
                    Save TimeCount
                  </button>
                </div>
              ) : (
